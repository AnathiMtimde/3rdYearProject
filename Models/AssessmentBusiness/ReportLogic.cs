...
namespace DGSappSem2.Models.AssessmentBusiness
{
    public class ReportLogic
    {
        ...
        public List<GraphicalReportVm> GetGraphicalReports(int classRoomId, int termId)
        {
            var db = new ApplicationDbContext();
            var subjects = db.Subjects.Where(o => o.ClassRoomID == classRoomId).ToList();
            var graphicalVms = new List<GraphicalReportVm>();
            foreach(var item in subjects)
            {
                var graphicalObj = new GraphicalReportVm();
                graphicalObj.label = item.SubjectName;
                graphicalObj.y = getSubjectAverage(item.SubjectID, termId, classRoomId);
                graphicalVms.Add(graphicalObj);
            }
            return graphicalVms;
        }
